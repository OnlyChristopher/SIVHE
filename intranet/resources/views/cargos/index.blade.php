            ...
            <div class="row page-titles">
                ...
                <div class="col-md-7 col-4 align-self-center">
                    <div class="text-right">
                        <button type="button" data-href="{{action('CargosController@create')}}" class="btn btn-sm waves-effect waves-light btn-info"> Crear
                            Nuevo Cargo
                        </button>
                    </div>
                </div>
                                ...
                                @if(count($cargos))
                                    <table class="table table-hover m-b-10">
                                        ...
                                        @foreach ($cargos as $cargo)
                                            <tr>
                                                <td>{{$cargo->idcargo}}</td>
                                                <td nowrap>{{$cargo->nombre}}</td>
                                                <td nowrap>{{$cargo->fecha_creacion}}</td>
                                                <td nowrap>
                                                    <form action="{{ route('cargos.destroy', $cargo->idcargo) }}"
                                                          method="post">
                                                        @if( Auth::user()->profile == 1|| Auth::user()->profile == 2)
                                                            <div class="btn-group">

                                                                <button type="button" class="btn btn-sm btn-icon btn-warning"
                                                                        data-href="{{route('cargos.edit',$cargo->idcargo)}}"
                                                                ...
                                                                @csrf
                                                                @method('DELETE')
                                                                <button id="btn-cargos-delete" type="submit" style="display: none"
                                                                        class="btn btn-sm btn-icon btn-danger"><i
                                                                            class="fa fa-trash-alt"></i>
                                                                </button>
                                                                   <button type="button" data-click="swal-danger" data-backdrop="btn-cargos-delete"
